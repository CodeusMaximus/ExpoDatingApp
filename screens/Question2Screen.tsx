...
const Question2Screen = ({ navigation, route }) => {
    const { email, firstName, location, age, city, state,  gender, interests, bio, images, password, answers } = route.params;
    ...
    const handleNext = () => {
      ...
      if (updatedAnswers.length < 15) {
        navigation.navigate('Question3', { email, age, firstName, location, city, state,  gender, interests, bio, images, password, answers: updatedAnswers });
      } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>Would you date someone who has kids?</Text>
      <TextInput
        style={styles.input}
